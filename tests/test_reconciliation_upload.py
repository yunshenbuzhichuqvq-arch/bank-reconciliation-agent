...
def test_upload_reconciliation_files_returns_excel_row_counts(tmp_path: Path) -> None:
    ...
    assert body["data"]["total_bank_rows"] == len(bank_df)
    assert body["data"]["total_clear_rows"] == len(clear_df)
    assert body["data"]["auto_fixed_rows"] == expected_auto_fixed
    assert body["data"]["pending_ai_rows"] == 0
    assert body["data"]["pending_human_rows"] == len(expected_exceptions)
    ...
    assert status_body["pending_human_rows"] == len(expected_exceptions)
    assert status_body["unresolved_rows"] == len(expected_exceptions)
    ...
    assert exceptions_body["total"] == len(expected_exceptions)
    ...
    assert persisted_status.auto_fixed_rows == expected_auto_fixed
    assert persisted_status.pending_ai_rows == 0
    assert persisted_status.pending_human_rows == len(expected_exceptions)
    assert persisted_status.unresolved_rows == len(expected_exceptions)
    ...
    assert persisted_transactions.count_bank_rows(user_id="demo_user", task_id=task_id) == len(bank_df)
    assert persisted_transactions.count_clear_rows(user_id="demo_user", task_id=task_id) == len(clear_df)
    ...
    assert bank_f2003["summary"]
    assert clear_f2003["summary"]

    persisted_queue = QueueService()
    assert persisted_queue.count_rows(user_id="demo_user", task_id=task_id) == len(
        expected_exceptions
    )
...
def test_upload_reconciliation_files_rejects_duplicate_bank_flow_id(
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == f"bank_file contains duplicate flow_id: {duplicated_flow_id}"
...
def test_upload_reconciliation_files_rejects_duplicate_clear_flow_id(
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == f"clear_file contains duplicate flow_id: {duplicated_flow_id}"
...
def test_reconciliation_service_builds_structured_match_results(tmp_path: Path) -> None:
    ...
    assert set(EXPECTED_BRANCHES) <= set(results_by_flow_id)
    expected_auto_fixed = set(results_by_flow_id) - set(EXPECTED_BRANCHES)
    assert expected_auto_fixed
    assert all(results_by_flow_id[flow_id].status == "AUTO_FIXED" for flow_id in expected_auto_fixed)
...
def test_reconciliation_service_build_match_results_passes_scenario_type(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "bank_reconciliation_agent.services.reconciliation.input.exception_router.classify",
        fake_classify,
    )
