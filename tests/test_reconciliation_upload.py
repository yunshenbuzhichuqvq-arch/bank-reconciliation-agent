...
from scripts.generate_mock_excel import SOURCE_A_COLUMNS, SOURCE_B_COLUMNS, generate_mock_excel
...
def test_upload_reconciliation_files_returns_excel_row_counts(tmp_path: Path) -> None:
    ...
    with source_a_path.open("rb") as source_a_file, source_b_path.open("rb") as source_b_file:
        response = client.post(
            ...
            data={"scenario_type": "BANK_ENTERPRISE"},
            files={
                "source_a_file": (
                    "source_a_enterprise_book.xlsx",
                    source_a_file,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                ),
                "source_b_file": (
                    "source_b_bank_statement.xlsx",
                    source_b_file,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                ),
            },
    ...
    assert body["data"]["scenario_type"] == "BANK_ENTERPRISE"
    assert body["data"]["total_source_a_rows"] == 10
    assert body["data"]["total_source_b_rows"] == 10
    ...
    assert status_body["scenario_type"] == "BANK_ENTERPRISE"
    ...
    assert exceptions_by_flow_id["F1004"]["source_a_amount"] == "300.00"
    assert exceptions_by_flow_id["F1004"]["source_b_amount"] == "295.00"
    ...
    assert 0 < exceptions_by_flow_id["F1004"]["audit_decision"]["confidence"] <= 0.85
    assert exceptions_by_flow_id["F1005"]["status"] == "PENDING_HUMAN"
    assert exceptions_by_flow_id["F1005"]["error_type"] == "BANK_UNARRIVED"
    assert exceptions_by_flow_id["F1005"]["source_a_amount"] == "120.00"
    assert exceptions_by_flow_id["F1005"]["source_b_amount"] is None
    assert exceptions_by_flow_id["F1006"]["status"] == "PENDING_HUMAN"
    assert exceptions_by_flow_id["F1006"]["error_type"] == "BOOK_UNRECORDED"
    assert exceptions_by_flow_id["F1006"]["source_a_amount"] is None
    assert exceptions_by_flow_id["F1006"]["source_b_amount"] == "45.00"

    ledger_response = client.get(
        f"/api/v1/ledger?task_id={task_id}&scenario_type=BANK_ENTERPRISE",
        headers=DEMO_HEADERS,
    )
    ...
    assert ledger_by_flow_id["F1004"]["scenario_type"] == "BANK_ENTERPRISE"
    ...
    assert 0 < float(ledger_by_flow_id["F1004"]["ai_confidence"]) <= 0.85
    ...
    persisted_status = ReconciliationService().get_status(task_id, user_id="demo_user")
    assert persisted_status.status == "AI_RUNNING"
    assert persisted_status.scenario_type == "BANK_ENTERPRISE"
    ...
    persisted_ledger_page = LedgerService().list(
        LedgerQuery(task_id=task_id, user_id="demo_user", scenario_type="BANK_ENTERPRISE")
    )
    ...
    assert persisted_transactions.count_source_a_rows(task_id, user_id="demo_user") == 10
    assert persisted_transactions.count_source_b_rows(task_id, user_id="demo_user") == 10
    source_a_f1004 = persisted_transactions.get_source_a_row(task_id, "F1004", user_id="demo_user")
    source_b_f1004 = persisted_transactions.get_source_b_row(task_id, "F1004", user_id="demo_user")
    assert source_a_f1004 is not None
    assert source_b_f1004 is not None
    assert all(column in source_a_f1004 for column in SOURCE_A_COLUMNS)
    assert all(column in source_b_f1004 for column in SOURCE_B_COLUMNS)
    assert source_a_f1004["scenario_type"] == "BANK_ENTERPRISE"
    assert source_b_f1004["scenario_type"] == "BANK_ENTERPRISE"
    assert source_a_f1004["amount"] == Decimal("300.00")
    assert source_b_f1004["amount"] == Decimal("295.00")
    assert source_b_f1004["bank_serial_no"] == "B202605211004"
    assert source_a_f1004["credit_amount"] == Decimal("300.00")
    assert source_a_f1004["remark"] == "金额差错样例"
    assert source_b_f1004["transaction_amount"] == Decimal("295.00")
    assert source_b_f1004["remark"] == "金额差错样例"
    assert source_a_f1004["summary"] == "企业账簿记账"
    assert source_b_f1004["summary"] == "银行到账"
    ...
    assert "source_a_amount=300.00" in rag_log_f1004["query_text"]
...
def test_upload_reconciliation_files_rejects_missing_required_bank_columns(
    ...
    source_a_path, source_b_path = generate_mock_excel(tmp_path)
    source_a_df = pd.read_excel(source_a_path).drop(columns=["voucher_no"])
    invalid_source_a = BytesIO()
    source_a_df.to_excel(invalid_source_a, index=False)
    invalid_source_a.seek(0)

    with source_b_path.open("rb") as source_b_file:
        response = client.post(
            "/api/v1/reconcile/upload",
            headers=DEMO_HEADERS,
            data={"scenario_type": "BANK_ENTERPRISE"},
    ...
    assert "source_a_file missing required columns" in response.json()["detail"]
    assert "voucher_no" in response.json()["detail"]
...
def test_upload_reconciliation_files_rejects_duplicate_bank_flow_id(
    ...
    source_a_df.loc[1, "flow_id"] = source_a_df.loc[0, "flow_id"]
    invalid_source_a = BytesIO()
    source_a_df.to_excel(invalid_source_a, index=False)
    invalid_source_a.seek(0)
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == "source_a_file contains duplicate flow_id: F1001"
...
def test_upload_reconciliation_files_rejects_duplicate_clear_flow_id(
    ...
    source_b_df.loc[1, "flow_id"] = source_b_df.loc[0, "flow_id"]
    invalid_source_b = BytesIO()
    source_b_df.to_excel(invalid_source_b, index=False)
    invalid_source_b.seek(0)

    with source_a_path.open("rb") as source_a_file:
        response = client.post(
            "/api/v1/reconcile/upload",
            headers=DEMO_HEADERS,
            data={"scenario_type": "BANK_ENTERPRISE"},
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == "source_b_file contains duplicate flow_id: F1001"
...
def test_upload_reconciliation_files_rejects_empty_bank_flow_id(
    ...
    source_a_path, source_b_path = generate_mock_excel(tmp_path)
    source_a_df = pd.read_excel(source_a_path)
    source_a_df.loc[0, "flow_id"] = None
    invalid_source_a = BytesIO()
    source_a_df.to_excel(invalid_source_a, index=False)
    invalid_source_a.seek(0)
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == "source_a_file contains empty flow_id values"
...
def test_reconciliation_service_builds_structured_match_results(tmp_path: Path) -> None:
    ...
    results = ReconciliationService()._build_match_results(source_a_df, source_b_df)
    ...
    assert results_by_flow_id["F1004"].source_a_amount == Decimal("300.00")
    assert results_by_flow_id["F1004"].source_b_amount == Decimal("295.00")
    assert results_by_flow_id["F1004"].amount_diff == Decimal("5.00")
    assert results_by_flow_id["F1005"].status == "PENDING_HUMAN"
    assert results_by_flow_id["F1005"].error_type == "BANK_UNARRIVED"
    assert results_by_flow_id["F1005"].source_a_amount == Decimal("120.00")
    assert results_by_flow_id["F1005"].source_b_amount is None
    assert results_by_flow_id["F1006"].status == "PENDING_HUMAN"
    assert results_by_flow_id["F1006"].error_type == "BOOK_UNRECORDED"
    assert results_by_flow_id["F1006"].source_a_amount is None
    assert results_by_flow_id["F1006"].source_b_amount == Decimal("45.00")
