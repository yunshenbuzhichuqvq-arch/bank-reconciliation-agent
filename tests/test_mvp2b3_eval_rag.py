...
# ---------------------------------------------------------------------------
# TASK-30.4: shared query enrichment helper integrated into the eval candidate path
# ---------------------------------------------------------------------------

from bank_reconciliation_agent.rag import query_enrichment  # noqa: E402


class _AnyQueryRetrieverWithStore:
    def __init__(self, embedding_backend: str = "hash") -> None:
        self.store = _StubStore(embedding_backend)
        self.requests: list = []

    def search(self, request) -> RagSearchResponse:
        self.requests.append(request)
        return RagSearchResponse(items=[_item("c1")])


def _enricher() -> query_enrichment.QueryEnricher:
    return query_enrichment.QueryEnricher.from_path(query_enrichment.DEFAULT_PROFILE_PATH)


def _target_and_control_cases() -> list["eval_rag.EvalCase"]:
    return [
        eval_rag.EvalCase(
            id="ent-1",
            scenario_type="BANK_ENTERPRISE",
...
def test_request_for_eval_mode_uses_query_override() -> None:
    case = eval_rag.EvalCase(
        id="ov-1",
        scenario_type="BANK_CLEARING",
        error_type="SINGLE_SIDE_MISSING",
    ...
    request = eval_rag.request_for_eval_mode(
        case, mode="dense", top_k=5, query_override="original enriched"
    )
    assert request.query == "original enriched"
    assert request.scenario_type == "BANK_CLEARING"


def test_matrix_enricher_enables_metadata_and_latency() -> None:
    captured: list[_AnyQueryRetrieverWithStore] = []

    def factory(backend: str) -> _AnyQueryRetrieverWithStore:
        r = _AnyQueryRetrieverWithStore(embedding_backend=backend)
        captured.append(r)
        return r

    report = eval_rag.evaluate_backend_mode_matrix(
        _target_and_control_cases(),
        requested_backends=["hash"],
        modes=["dense"],
        real_backend_policy="auto",
        retriever_factory=factory,
        query_enricher=_enricher(),
        enrichment_profile_id="bank-clearing-single-side-missing",
        query_profile_sha256="profile-hash-123",
    )

    qe = report["query_enrichment"]
    assert qe["enabled"] is True
    assert qe["profile"] == "bank-clearing-single-side-missing"
    assert qe["profile_sha256"] == "profile-hash-123"
    latency = qe["latency_ms"]
...
def test_matrix_enricher_applies_only_to_target_case() -> None:
    captured: list[_AnyQueryRetrieverWithStore] = []
    ...
    eval_rag.evaluate_backend_mode_matrix(
        _target_and_control_cases(),
        requested_backends=["hash"],
        modes=["dense"],
        real_backend_policy="auto",
    ...
    queries = {r.query for cap in captured for r in cap.requests}
    assert "企业端查询" in queries
    assert any(q.startswith("清算端查询 ") and q != "清算端查询" for q in queries)
    assert "清算端查询" not in queries


def test_matrix_without_enricher_leaves_queries_unchanged() -> None:
    captured: list[_AnyQueryRetrieverWithStore] = []
    ...
    assert report["query_enrichment"]["enabled"] is False
    queries = {r.query for cap in captured for r in cap.requests}
    assert queries == {"企业端查询", "清算端查询"}


def test_matrix_cli_query_enrichment_profile_opt_in(tmp_path: Path) -> None:
    matrix_json_path = tmp_path / "rag_quality_matrix.json"
    eval_rag.main(
        [
            "--eval-set",
            ...
            "--real-backend-policy",
            "auto",
            "--query-enrichment-profile",
            "bank-clearing-single-side-missing",
            "--matrix-report",
            str(tmp_path / "rag_quality_matrix.md"),
            "--matrix-json",
            str(matrix_json_path),
    ...
    payload = json.loads(matrix_json_path.read_text(encoding="utf-8"))
    qe = payload["query_enrichment"]
    assert qe["enabled"] is True
    assert qe["profile"] == "bank-clearing-single-side-missing"
    assert isinstance(qe["profile_sha256"], str) and qe["profile_sha256"]
    assert qe["latency_ms"]["count"] == payload["case_count"]
