...
## ADR-053: ReportAgent 失败降级 —— 模板兜底 + schema 校验 + 有界重试
...
**A. 模板降级(选定)** —— ReportAgent 镜像 AuditAgent:provider 注入、fake 默认确定性、Pydantic schema 校验 + 有界重试;耗尽则降级为确定性模板叙述,`llm_used=false`。数字区块照常(代码渲染,不依赖 LLM)。
...
选 A。复用 ADR-005/007/022/008 既有管线:fake 默认 + schema 校验 + 有界重试 + 模板降级 + structlog(带 `prompt_version`)。报告永远可出,降级时诚实标注。
