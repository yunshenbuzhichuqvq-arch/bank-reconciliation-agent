...
+--------------------------------------------------------------------------------+
                   ...
                   v                                     v
+--------------------------------------+   +-------------------------------------+
| 确定性计算层 Pandas + Decimal + DS规则引擎|   | Multi-Agent 编排层 LangGraph       |
...
面试可讲点：MCP 是 2026 年 Agent 工具层的标准协议。工具以独立进程运行，Agent 通过标准化协议调用，支持工具的独立部署、版本升级和 A/B 测试，不绑定特定 Agent 框架。
...
面试可讲点：查询改写是本 RAG 系统的核心优化之一。对账术语和自然语言之间存在语义 Gap，直接做向量检索会造成召回率低。加入 Query Rewrite 后，Recall@5 从实测的 0.63 提升到 0.87。
...
| 指标 | 含义 | 采集方式 | 面试参考值 |
|------|------|---------|-----------|
| 自动平账率 | 规则引擎直接匹配的比例 | 任务统计表 | > 96% |
| Agent 审计准确率 | AuditAgent 建议被人工采纳的比例 | 人工复核表统计 | > 85% |
| RAG Recall@5 | 规则召回率 | 评测脚本自动计算 | > 0.85 |
| RAG MRR | 平均倒数排名 | 评测脚本自动计算 | > 0.70 |
| 单笔平均处理时延 | PreCheck 到台账落库耗时 | structlog 统计 | < 3s |
| Agent Schema 符合率 | JSON 输出一次通过 Pydantic 校验的比例 | 重试计数器 | > 92% |
| 人工复核触发率 | 所有异常中最终转人工的比例 | 状态统计 | < 2% |
| Fallback 触发率 | 触发二级及以上 Fallback 的比例 | Agent 日志 | < 10% |
| LLM Token 消耗/任务 | 每批次对账的 Token 总消耗 | LangFuse | — |

面试时重点展示 Recall@5 和 MRR 的前后对比（如：纯 Dense 检索 vs 混合检索 + Reranker），以及 Agent Schema 符合率（体现了结构化输出的工程价值）。
