...
- Related: src/bank_reconciliation_agent/services/live_registry.py, src/bank_reconciliation_agent/services/stream_emitter.py, src/bank_reconciliation_agent/services/reconciliation.py, src/bank_reconciliation_agent/api/v1/reconcile.py, decisions/ADR-045(两段式 start-live + by-taskId SSE,本 ADR 修其实现层生命周期), decisions/ADR-037(事件 schema 不变), decisions/ADR-044(前端 dual-path 不变), decisions/ADR-036(双路径 SSE / QueueEmitter)
...
落地参数:`LIVE_EMITTER_TTL_SECONDS = 60`(finished 后保留期)、`LIVE_EMITTER_MAX_AGE_SECONDS = 3600`(未完成 emitter 注册超长上限,防后台 hang 泄漏);`_sweep()` 在 `register` / `get_emitter` 入口惰性触发;events 正常消费完 task_done 后 `unregister`(消费完即回收,幂等 `pop`)。
...
- 旧契约"后台结束即清理 emitter"失效 → `test_start_live_marks_running_emits_task_progress_and_cleans_registry` 清理断言已更新为新契约(有意变更,非破坏)。
- 二次订阅边界:emitter 被消费 / 回收后再订阅仍 404;若需"已完成可重看",叠加 C(follow-up)。
- registry 从纯 dict 增加 created_at / finished 状态与惰性 sweep。

可观测(防回归历史 SSE 静默降级 gap)——**本 fix 内 deferred,记为 follow-up(2026-06-18 review 定)**:
- 现状:events 端点对"命中 live emitter / 404 未命中"路径打结构化日志的约束,**未在本 fix 实现**。
- 理由:本 fix 保留 ADR-045 红线 #2(订阅无 emitter → 404,无 DB 回放 fallback),故当前不存在"实时静默降级为回放"的面,该日志的防回归价值主要在未来叠加方案 C(DB 回放)时显现;为把竞态修复的改动面压到最小,随 C 一并落地。
- 负向(诚实记录,非"已解决"):在引入 C 之前,events 命中/404 两路缺结构化日志,线上排查 live 链路(如本次 404 竞态)缺一手观测信号——这是本次有意接受的小盲区。
- 硬约束(仍生效,不可降级):未来叠加方案 C 前,**必须先落地该结构化日志 + 显式区分"实时 vs DB 回放"**;违反即把真实时降级为回放(ADR-045 红线),非可选优化。
