...
error_ledger_table = Table(
    ...
    UniqueConstraint("user_id", "task_id", "flow_id", name="uk_ledger_user_task_flow"),
...
class LedgerService:
    ...
    def list(self, *, user_id: str, query: LedgerQuery) -> Page[LedgerRow]:
        """根据查询条件返回差错台账分页结果。"""
        self._ensure_initialized()
        filters = self._build_filters(user_id, query)
    ...
    def _build_filters(self, user_id: str, query: LedgerQuery) -> list[object]:
        filters: list[object] = [error_ledger_table.c.user_id == user_id]
        if query.task_id:
