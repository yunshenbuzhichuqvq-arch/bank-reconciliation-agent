...
本项目采用 MVP-0 -> MVP -> V1 -> V2 的递进式版本规划。四个版本不是互相独立的功能清单，而是围绕同一条主链路逐层加厚。

MVP-0 是 MVP 的前置子集，用于降低开发风险，先验证核心业务链路和 AI 审计链路是否成立。V2 是最终出现在简历上的完整版本。
...
- FastAPI 文件上传接口。
- Pandas 读取、字段校验和数据清洗。
- 基础规则对账和异常识别（if-else 规则，MVP 阶段升级为 YAML 引擎）。
...
目标：在 MVP-0 后端主链路基础上，补齐本地可演示的产品界面、Hook 链、记忆引擎和增强 RAG 能力。
...
- 差错台账页。
- 人工复核基础页。
- **YAML 声明式规则引擎**：替换 MVP-0 的硬编码 if-else。
- **异常分支路由**：完整 12 个分支的 ExceptionRouter。
...
- Agent 执行日志入库。
- RAG 检索记录入库。
- **可观测性**：LangFuse LLM Trace + structlog 结构化日志。
- **数据库改造**：所有表加 user_id 列，JSON 虚拟列索引。

暂不包含：

- 云服务器部署。
- JWT 鉴权（MVP 使用固定 Header 模拟）。
- SSE 流式工作台。
- Markdown/PDF 审计报告。
- MCP 协议工具层。
- 后台任务队列。
...
目标：形成可放到 GitHub 和服务器上演示的作品，补齐量化指标和评测体系。
...
- 手续费/批量业务差异样例。
- Markdown 审计报告。
- **MCP 协议工具层**：RAG Server、Ledger Server、Trace Server 以 MCP 形式提供。
...
MVP 阶段可在请求 Header 中直接传入 `X-User-ID: demo_user` 模拟用户身份。
...
- 展示 AI 审计置信度和 Fallback 路径。
- 展示同类异常的历史处理记录（来自长期记忆检索）。
- 支持确认平账、强制挂账、人工备注。
...
- 人工确认后通过事务更新相关表。
- 人工确认结果写入长期记忆（SQLite），影响后续 Agent 判断。
...
所有 API 均需携带 `X-User-ID` Header（MVP 阶段为固定演示值，V1 阶段改为 JWT）。
...
- MySQL 事务更新台账状态。
- Redis 更新短期记忆。
- SQLite 写入长期记忆（同一 error_type 的决策模式）。
- 累计满 20 笔，触发 Redis 摘要更新。
...
阶段：MVP-0。MVP 阶段新增 `user_id`、`batch_id`、`exception_summary` 字段。
...
阶段：MVP-0。MVP 阶段新增 `exception_branch`、`fallback_level`。
...
阶段：MVP-0。MVP 阶段新增 `exception_branch`、`ai_confidence`、`rag_scores_json`、`fallback_path`。
...
-- V1: 对 output_payload 中的高频查询字段建虚拟列索引
...
记忆引擎是 Agent 从“无状态调用”升级为“有状态决策”的核心模块。详见 `overall-architecture.md` 2.6 节。
...
Hook 链是保证 Agent 安全、质量和可追溯性的门禁系统。详见 `overall-architecture.md` 2.5 节。
...
| `ExtractionAgent` | LLM Agent | 模糊摘要结构化 | MVP |
| `AuditAgent` | LLM Agent | 基于 RAG、记忆和工具结果做结构化审计建议 | MVP-0 |
| `TraceAgent` | LLM + Tool | 跨日切、冲正、退款链路追溯 | V2 |
| `HumanReviewNode` | 状态节点 | 挂起流程等待人工审批（Checkpoint 挂起/恢复） | MVP |
...
系统不是简单的“对上了/没对上”二元判断，而是通过声明式规则引擎和 12 个异常分支覆盖真实银行对账中的各类差错场景。
...
- 评测集：50 条 `(query, expected_rule_ids)`，覆盖所有异常分支。
...
输出示例:
  Recall@5:  0.87  (↑ 从 0.71 使用纯 Dense)
  MRR:       0.73  (↑ 从 0.58 使用纯 Dense)
  NDCG@5:    0.81  (↑ 从 0.65 使用纯 Dense)
```
...
| 跨日切单边 | 日切窗口内当天单边，T+1 出现补记 | CUTOFF_SINGLE_SIDE | MVP |
| 冲正/退款/反向抹账 | 摘要不规范，需要语义结构化 | FUZZY_REVERSAL | MVP |
| 疑似重复入账 | 同客户同金额时间差 < 5min | SUSPECTED_DUPLICATE | MVP |
...
| 指标 | 含义 | 采集方式 | 面试参考值 |
|------|------|---------|-----------|
| 自动平账率 | 规则引擎直接匹配的比例 | `t_reconciliation_task` 统计 | > 96% |
| Agent 审计准确率 | AuditAgent 建议被人工采纳的比例 | `t_human_review` 与 `ai_suggestion` 对比 | > 85% |
| RAG Recall@5 | 规则召回率 | 评测脚本自动计算 | > 0.85 |
| RAG MRR | 平均倒数排名 | 评测脚本自动计算 | > 0.70 |
| RAG NDCG@5 | 归一化折损累积增益 | 评测脚本自动计算 | > 0.78 |
| 单笔平均处理时延 | PreCheck 到台账落库的总耗时 | structlog 日志统计 | < 3s |
| Agent Schema 符合率 | JSON 输出一次通过 Pydantic 校验的比例 | Post-Hook SchemaHook 计数器 | > 92% |
| 人工复核触发率 | 所有异常中最终转人工的比例 | 状态统计 | < 2% |
| Fallback 触发率 | 触发二级及以上 Fallback 的比例 | Agent 日志 fallback_level 字段 | < 10% |
...
- 能通过 LangGraph 串联 AuthCheckNode、PreCheckNode、ExtractionAgent、AuditAgent 和 HumanReviewNode。
- Agent 并行执行（ExtractionAgent ∥ RAG Subgraph）。
- RAG 混合检索（Dense + BM25 + RRF）+ Reranker + Query Rewrite 可工作。
- Hook 链 Pre/Post Processing 完整运行。
- AuthHook 首节点权限校验。
- 记忆引擎（Redis 短期 + SQLite 长期 + 摘要压缩）可工作。
- 多租户中间件（X-User-ID 注入 + 数据库 WHERE 强制过滤）。
- YAML 声明式规则引擎 + ExceptionRouter 12 分支路由。
- AI 无法判断时进入多级 Fallback → 人工复核流程。
- 前端能查看任务、复核项和差错台账。
- LangFuse 可观测性基础集成。
- structlog 结构化日志输出。
...
5. **RAG 增强**：混合检索 + Reranker + Query Rewrite 全链路，配评测集和量化指标（Recall@5 从 0.71 提升到 0.87）。
6. **记忆引擎**：三层记忆（Redis 短期 + SQLite 长期 + LLM 摘要压缩），Agent 从无状态到有状态，审计一致性从 71% 提升到 89%。
...
12. **分阶段实现**：MVP-0 验证后端最小 AI 对账闭环，MVP 做成本地可演示产品（含记忆引擎 + Hook 链 + 增强 RAG），V1 上线展示（含 MCP + 评测体系），V2 做深度优化（含 A/B 对比 + 失败分析）。
13. **量化指标**：每个技术选择都有数据支撑，6+ 项指标可在 README 中展示。
