...
因此，PRD 中的复杂能力按阶段落地。MVP-0 只验证主链路；MVP 做本地演示所需的最小生产化能力；V1/V2 再补评测、部署、MCP、A/B 和压力测试。所有未实际跑出的指标均作为目标口径，不在简历和面试中表述为真实生产数据。
...
每个阶段都必须有可验收产物。MVP-0 的产物是 API、数据库记录和 RAG/Agent JSON；MVP 的产物是本地页面、人工复核流和 Hook/Memory 日志；V1 的产物是部署地址、SSE 演示、评测报告和指标仪表板；V2 的产物是失败样本分析、A/B 对比和压力测试报告。
...
目标：在 MVP-0 后端主链路基础上，补齐本地可演示的产品界面，并实现 Hook 链、记忆引擎和增强 RAG 的最小可运行版本。MVP 阶段追求“能证明设计成立”，不追求完整企业级能力。
...
- Reranker 可先做可开关能力，保证无 Reranker 时主链路仍可运行。
...
输出示例:
  Recall@5:  0.87  (目标示例，实际以评测脚本输出为准)
  MRR:       0.73  (目标示例，实际以评测脚本输出为准)
  NDCG@5:    0.81  (目标示例，实际以评测脚本输出为准)
```
...
| RAG 检索 | 检索调试接口返回、命中 chunk、相似度分数 | MVP-0/MVP |
| Hook 链 | structlog 日志中的 pre_hook/post_hook 结果 | MVP |
| 人工复核 | 复核页面截图、人工审批记录、台账状态变化 | MVP |
| 记忆引擎 | 同类异常历史记录查询、MemoryUpdate 日志 | MVP |
| 可观测性 | LangFuse Trace 或本地 JSON trace 文件 | MVP/V1 |
| RAG 评测 | `reports/rag_eval.md` 和 `data/rag_eval_set.json` | V1 |
| Schema 测试 | Pytest 输出和失败样本 | V1 |
| 部署能力 | Docker Compose 启动截图、部署地址、README | V1 |
| 失败分析 | 失败样本表、原因分类、下一步优化建议 | V2 |
